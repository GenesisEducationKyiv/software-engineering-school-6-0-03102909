...
export class ScannerService {
    ...
    for (const repo of repositories) {
      try {
        ...
        if (latestTag === repo.lastSeenTag) {
          this.log.debug({ repo: `${repo.owner}/${repo.name}`, tag: latestTag }, 'no new release found');
          continue;
        }
  ...
  private async notifySubscribers(
    ...
    this.log.info({ repo: repoFullName, tag, subscriberCount: subscribers.length }, 'notifying subscribers about release');
    ...
    this.log.info({ repo: repoFullName, tag, successCount, failCount }, 'finished notifying subscribers');
  }
}
