...
describe('RabbitMQ Messaging', () => {
  ...
  describe('connectRabbitMQ', () => {
    it('should connect and assert expected topology', async () => {
      ...
      expect(mockConfirmChannel.assertExchange).toHaveBeenCalledWith(DLX_EXCHANGE, 'fanout', {
        durable: true,
      });
      ...
      expect(mockConfirmChannel.assertExchange).toHaveBeenCalledWith(EXCHANGE_NAME, 'direct', {
        durable: true,
      });
  ...
  describe('consumeQueue', () => {
    ...
    it('should throw error if channel is not initialized', async () => {
      await disconnectRabbitMQ(mockLogger);
      await expect(consumeQueue('test-queue', schema, vi.fn(), mockLogger)).rejects.toThrow(
